...
class DocumentRegistry:
    ...
    def register(self, result: AcquisitionResult) -> tuple[SourceDocument, Path]:
        ...
        for path in versions:
            ...
            if result.content_hash and data.get("content_hash") == result.content_hash:
                return SourceDocument(**data), path
    ...
    def verify(self, document: SourceDocument) -> bool:
        if not document.content_hash or not document.blob_path:
            return document.status.value == "metadata_only"
        blob = self.root / document.blob_path
        return blob.is_file() and sha256(blob.read_bytes()).hexdigest() == document.content_hash
