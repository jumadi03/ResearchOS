...
from app.knowledge.modeling.models import GraphLifecycleProjection
from app.knowledge.publication.engine import PublicationEngine
from app.knowledge.publication.models import (
    PublicationKind, PublicationRelationship, PublicationRelationType,
)
from app.knowledge.publication.persistence import (
    PublicationRelationshipStore, PublicationStore,
)
...
from app.knowledge.theory.models import TheoryDependencyImpact, TheoryReviewState
...
class KnowledgeTheoryPipeline:
    def __init__(
        ...
        object_store=None, semantic_relation_resolver=None,
    ) -> None:
        self.graphs = graphs
        self.data_repository = data_repository
        self.object_store = object_store
        self.semantic_relation_resolver = semantic_relation_resolver
        ...
        self.cross_study_store = CrossStudyPropositionStore(
            output_root / "cross-study-propositions"
        )
        self.cross_study_propositions = {
            item.proposition_id: item
            for item in self.cross_study_store.load_all()
        }
        ...
        self.publication_relationship_store = PublicationRelationshipStore(
            output_root / "publication-relationships"
        )
        self.bundles = {
        ...
        self.publication_relationships = {
            ...
            for item in self.publication_relationship_store.load_all()
        }
    ...
    def propose_cross_study_proposition(
        ...
        if not statement.strip() or not rationale.strip():
            raise ValueError(
                "Cross-study proposition statement and rationale are required"
            )
        evidence = bind_cross_study_evidence(
            self.graphs, evidence_references,
        )
        identity = (
            f"{statement.strip()}:{','.join(f'{item.graph_id}:{item.object_id}:{item.stance.value}' for item in evidence)}:"
            f"{proposed_by}:{proposed_at}"
        )
        item = CrossStudyProposition(
            "cross-study-proposition-"
            + sha256(identity.encode()).hexdigest()[:24],
            statement.strip(), evidence, proposed_by, rationale.strip(),
            proposed_at,
        ).finalized()
        existing = self.cross_study_propositions.get(item.proposition_id)
        if existing is not None:
            if existing.content_hash != item.content_hash:
                raise RuntimeError("Cross-study proposition conflict")
            return existing, self.cross_study_store.save(existing)
        self.cross_study_propositions[item.proposition_id] = item
        return item, self.cross_study_store.save(item)

    def review_cross_study_proposition(
        self, proposition_id: str, *, decision: str, reviewer: str,
        rationale: str, occurred_at: str,
    ):
        item = self.cross_study_propositions.get(proposition_id)
        if item is None:
            raise KeyError(
                f"Unknown cross-study proposition: {proposition_id}"
            )
        self._revalidate_cross_study_proposition(item)
        reviewed = item.review(
            decision=decision, reviewer=reviewer, rationale=rationale,
            occurred_at=occurred_at,
        )
        self.cross_study_propositions[proposition_id] = reviewed
        return reviewed, self.cross_study_store.save(reviewed)

    def build_cross_study_proposition_theory(
        self, proposition_id: str, *, generated_by: str,
    ):
        item = self.cross_study_propositions.get(proposition_id)
        if item is None:
            raise KeyError(
        ...
        bundle = proposition_theory_bundle(
            item, created_at=DiscoveryRun.timestamp(),
        )
        ...
        if self.data_repository is not None:
            self.data_repository.persist_artifact(
                ...
                artifact_type="theory_bundle",
                title="Reviewed cross-study theory bundle", status="draft",
                metadata=asdict(bundle), actor_id=generated_by,
    ...
    def list_cross_study_propositions(self):
        return tuple(sorted(
            self.cross_study_propositions.values(),
            key=lambda item: item.proposition_id,
        ))

    def _revalidate_cross_study_proposition(self, item):
        references = tuple(
            (ref.graph_id, ref.object_id, ref.stance.value)
            for ref in item.evidence
        )
        current = bind_cross_study_evidence(self.graphs, references)
        if current != item.evidence:
            raise ValueError(
                "Cross-study proposition evidence provenance has changed"
            )
        if self.data_repository is None:
            raise ValueError(
                "Canonical repository is required for proposition review"
            )
        for graph_id in sorted({ref.graph_id for ref in item.evidence}):
            graph = self.graphs[graph_id]
            object_ids = tuple(sorted(
                node.provenance.object_id for node in graph.nodes
            ...
            lifecycle = self.graph_lifecycle(graph_id)
            if (
                not lifecycle.current
                or lifecycle.impacted_semantic_relation_ids
            ):
                raise ValueError(
                    f"Cross-study proposition requires current graph: {graph_id}"
                )
    ...
    def build_theories(self, graph_ids: tuple[str, ...], *, generated_by: str):
        ...
        for graph in graphs:
            ...
            self.evidence_admission_gate.revalidate(graph, admissions)
            lifecycle = self.graph_lifecycle(graph.graph_id)
            if lifecycle.impacted_semantic_relation_ids:
                raise ValueError(
                    "Theory construction requires current graph dependencies: "
                    + ", ".join(lifecycle.impacted_semantic_relation_ids)
                )
        build = (
            self.theory_builder.build_cross_study
            if len(graphs) > 1 else self.theory_builder.build
        )
        bundle = build(graphs, created_at=DiscoveryRun.timestamp())
        self.bundles[bundle.bundle_id] = bundle
        if self.data_repository is not None:
            self.data_repository.persist_artifact(
                artifact_id=bundle.bundle_id, project_id=bundle.bundle_id,
    ...
    def validate_theories(self, bundle_id, **options):
        bundle = self._bundle(bundle_id)
        self._require_current_dependencies(bundle)
    ...
    def validation_history(self, bundle_id):
        bundle = self._bundle(bundle_id)
        dependencies_current = self.dependency_impact(bundle_id).current
        reports = sorted(
            (item for item in self.validation_reports.values()
             if item.theory_bundle_id == bundle_id),
            key=lambda item: (item.assessed_at, item.report_id), reverse=True,
        ...
        return tuple({
            **asdict(item),
            "active_for_current_bundle": (
                item.theory_bundle_hash == bundle.content_hash
    ...
    def publication_readiness(
        ...
        readiness = self.publication_engine.readiness(
            bundle, report, PublicationKind(kind)
        )
        return self._with_dependency_readiness(readiness, bundle)
    ...
    def preview_publication(
        ...
        bundle = self._bundle(bundle_id)
        report = self._publication_report(bundle, validation_report_id)
        preview = self.publication_engine.preview(
            bundle, report, PublicationKind(kind)
        )
        return self._with_dependency_readiness(preview, bundle)
    ...
    def dependency_impact(self, bundle_id):
        ...
        graph_projections = tuple(
            (
                self.graph_lifecycle(graph_id)
                if graph_id in self.graphs or self.data_repository is not None
                else
                GraphLifecycleProjection(
                    graph_id, "", "snapshot_only", True, (), (), (),
                    "embedded_snapshot_only",
                )
            )
            for graph_id in bundle.graph_ids
        )
        relation_states = tuple(sorted({
            state
            for projection in graph_projections
            for state in projection.semantic_relation_states
        }))
        relation_ids = tuple(item[0] for item in relation_states)
        impacted_relations = tuple(
            item[0] for item in relation_states if item[1] != "accepted"
        )
        if self.data_repository is None:
            impacted_graphs = tuple(
                item.graph_id for item in graph_projections if not item.current
            )
            return TheoryDependencyImpact(
                bundle.bundle_id, evidence_ids, (), (),
                bundle.graph_ids, impacted_graphs, tuple(
                    (item.graph_id, item.state) for item in graph_projections
                ),
                not impacted_graphs, "embedded_snapshot_only",
                relation_ids, impacted_relations, relation_states,
            )
        admissions = self.data_repository.resolve_evidence_admissions(evidence_ids)
        by_id = {item.evidence_object_id: item for item in admissions}
        states = tuple(
            (
                evidence_id,
                (
                    by_id[evidence_id].review_state.value
                    if evidence_id in by_id
                    and by_id[evidence_id].review_state is not None
                    else "missing"
                ),
            )
        ...
        impacted = tuple(
            evidence_id for evidence_id, state in states if state != "accepted"
        )
        impacted_graphs = tuple(
            item.graph_id for item in graph_projections if not item.current
        )
        return TheoryDependencyImpact(
            bundle.bundle_id, evidence_ids, impacted, states,
            bundle.graph_ids, impacted_graphs,
            tuple((item.graph_id, item.state) for item in graph_projections),
            not impacted and not impacted_graphs,
            "canonical_evidence_and_semantic_relation_ledgers",
            relation_ids, impacted_relations, relation_states,
        )

    def graph_lifecycle(self, graph_id):
        graph = self.graphs.get(graph_id)
        if graph is None:
            raise KeyError(f"Unknown scientific knowledge graph: {graph_id}")
        evidence_ids = tuple(sorted({
            node.provenance.object_id
            for node in graph.nodes if node.provenance is not None
        } | {
            edge.provenance.object_id for edge in graph.edges
        }))
        relation_states = (
            tuple(self.semantic_relation_resolver(graph_id))
            if self.semantic_relation_resolver is not None else ()
        )
        relation_ids = tuple(item[0] for item in relation_states)
        impacted_relations = tuple(
            item[0] for item in relation_states if item[1] != "accepted"
        ...
        if self.data_repository is None:
            return GraphLifecycleProjection(
                graph.graph_id, graph.content_hash,
                "superseded" if impacted_relations else "snapshot_only",
                not impacted_relations,
                evidence_ids, (), (), "embedded_snapshot_only",
                relation_ids, impacted_relations, relation_states,
            )
        admissions = self.data_repository.resolve_evidence_admissions(evidence_ids)
        by_id = {item.evidence_object_id: item for item in admissions}
        ...
        return GraphLifecycleProjection(
            graph.graph_id, graph.content_hash,
            "superseded" if impacted or impacted_relations else "current",
            not impacted and not impacted_relations,
            evidence_ids, impacted, states,
            "canonical_evidence_and_semantic_relation_ledgers",
            relation_ids, impacted_relations, relation_states,
        )

    def _require_current_dependencies(self, bundle):
        impact = self.dependency_impact(bundle.bundle_id)
        if not impact.current:
            evidence = ", ".join(
                impact.impacted_evidence_ids
                or impact.impacted_semantic_relation_ids
            )
            raise ValueError(
                "Theory bundle has stale or inadmissible dependencies: "
                f"{evidence}"
            )
        return impact

    def _with_dependency_readiness(self, result, bundle):
        impact = self.dependency_impact(bundle.bundle_id)
        check = {
            "key": "current_dependencies",
            "label": "Current evidence dependencies",
            "passed": impact.current,
            "detail": (
                "All theory evidence and semantic relations remain accepted."
                if impact.current else
                "Revalidation is required because dependencies are no longer "
                "accepted: " + ", ".join(
                    impact.impacted_evidence_ids
                    or impact.impacted_semantic_relation_ids
                )
        ...
        checks = tuple(result["checks"]) + (check,)
        return {
            **result,
            "ready": bool(result["ready"] and impact.current),
            "checks": checks,
            "dependency_impact": asdict(impact),
        }
    ...
    def relate_publication(
        self, publication_id, *, relation_type, target_publication_id,
        actor_id, rationale, occurred_at,
    ):
        if publication_id not in self.publication_packages:
            raise KeyError(f"Unknown publication package: {publication_id}")
        relation = PublicationRelationType(relation_type)
        if relation is PublicationRelationType.RETRACTS:
            ...
            if not target_publication_id:
                raise ValueError(f"{relation.value} requires a target publication")
            if target_publication_id == publication_id:
                raise ValueError("Publication cannot correct or supersede itself")
            if target_publication_id not in self.publication_packages:
                raise KeyError(f"Unknown target publication package: {target_publication_id}")
        if not rationale.strip():
            raise ValueError("Publication relationship rationale is required")
        identity = (
            f"{publication_id}:{relation.value}:{target_publication_id or ''}:"
            f"{actor_id}:{occurred_at}"
        )
        item = PublicationRelationship(
            f"publication-relationship-{sha256(identity.encode()).hexdigest()[:24]}",
            publication_id, relation, target_publication_id, actor_id,
            rationale.strip(), occurred_at,
        ).finalized()
        if self.data_repository is not None:
            self.data_repository.record_publication_relationship(item)
        self.publication_relationships[item.relationship_id] = item
        return item, self.publication_relationship_store.save(item)

    def publication_lifecycle(self, publication_id):
        if publication_id not in self.publication_packages:
            raise KeyError(f"Unknown publication package: {publication_id}")
        relationships = tuple(sorted(
            (item for item in self.publication_relationships.values()
             if item.source_publication_id == publication_id
             or item.target_publication_id == publication_id),
            key=lambda item: (item.occurred_at, item.relationship_id),
        ))
        retraction = next((
            item for item in reversed(relationships)
            if item.source_publication_id == publication_id
            and item.relation_type is PublicationRelationType.RETRACTS
        ), None)
        inbound = next((
            item for item in reversed(relationships)
            if item.target_publication_id == publication_id
            and item.relation_type in {
                PublicationRelationType.CORRECTS,
                PublicationRelationType.SUPERSEDES,
            }
        ), None)
        state = (
            "retracted" if retraction else
            "corrected" if inbound and inbound.relation_type is PublicationRelationType.CORRECTS else
            "superseded" if inbound else "current"
        )
        return {
            "publication_id": publication_id, "state": state,
            "current": state == "current",
            "replacement_publication_id": inbound.source_publication_id if inbound else None,
            "relationships": tuple(asdict(item) for item in relationships),
        }
